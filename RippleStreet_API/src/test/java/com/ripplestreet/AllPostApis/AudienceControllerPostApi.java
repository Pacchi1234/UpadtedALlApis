...
public class AudienceControllerPostApi extends postApiutilities {
	...
	public void exportUserList() throws IOException {
		...
		response = RestAssured.given().contentType(ContentType.JSON).body(PutBody).when().post(
				"/community-service/v1/event/participant/export?eventId="
						+ eventId + "&participantType=Host");
