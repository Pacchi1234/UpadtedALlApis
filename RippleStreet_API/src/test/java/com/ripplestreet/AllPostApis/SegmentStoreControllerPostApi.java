...
public class SegmentStoreControllerPostApi extends postApiutilities {
	@Test(groups="segmentation")
	public void pushSegmentDataFetcherRequest() throws IOException {
		Testcase = 35;
		...
		response = RestAssured.given().header("Authorization", AccessToken).contentType(ContentType.JSON).body(PutBody).when()
				.post("/segmentation/v1/segment/store/geoCode");
