...
public class SegmentationJobControllerPostApi extends postApiutilities {
	@Test(groups="segmentation")
	public void createaSegmentJob() throws IOException {
		Testcase = 37;
		...
		response = RestAssured.given().contentType(ContentType.JSON).body(PutBody).when()
				.post("/segmentation/v2/segmentJobs");
