...
public class EnrollmentControllerPostApi extends postApiutilities{
	@Test(groups="community-service")
	public void RegitserUserEnrollment() throws IOException {
		Testcase = 18;
		...
		response = RestAssured.given().contentType(ContentType.JSON).body(PutBody).when()
				.post("/community-service/enroll/user");
	}
	@Test(groups="community-service")
	public void createNewEnrollment() throws IOException {
		Testcase = 19;
		File file = new File(postApipath);
		FileInputStream fis = new FileInputStream(file);
		@SuppressWarnings("resource")
		...
		response = RestAssured.given().contentType(ContentType.JSON).body(PutBody).when()
				.post("/community-service/enrollment");
